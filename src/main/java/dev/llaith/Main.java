...
/**
 * Main entry point for the TorrentScanner application.
 * <p>
...
public class Main implements Callable<Integer> {
    ...
    @Option(names = {"--generate-index"}, description = "Generate an index file with all torrent information", paramLabel = "<path-to-index-file>")
    protected String indexFilePath;

    @Option(names = {"--skip-scanning"}, description = "Skip scanning mode, only generate index if --generate-index is specified")
    ...
    public Integer call() {
        ...
        logger.info("Starting TorrentScanner application");
        logger.debug("Command line options: verbose={}, quiet={}, dry-run={}, clobber={}, generate-index={}, skip-scanning={}", 
                verbose, quiet, dryRun, clobber, indexFilePath, skipScanning);

        // Check if skip-scanning is passed without generate-index
        if (skipScanning && (indexFilePath == null || indexFilePath.isEmpty())) {
    ...
    protected void generateIndex() throws IOException {
        ...
        TorrentToCsvExporter.generateIndex(directoryPath, indexFilePath, dryRun, clobber);
    }
    ...
    protected int scanDirectory() throws IOException {
        return TorrentToCsvExporter.scanDirectory(directoryPath, outputDir, dryRun, clobber);
    }
    ...
    private void configureLogging() {
        ...
        Logger appLogger = loggerContext.getLogger("dev.llaith");
